...
select p.pet_ill_name,avg(p.pet_age),max(o.owner_age) from tbl_owner o
join tbl_pet p  on o.id=p.owner_id
...
select o.id ,p.product_price ,u.user_age from tbl_user u 
join tbl_order o 
on u.id =o.user_id and u.user_age =20 
join tbl_product p 
...
select m.* ,p.post_title  from tbl_member m left outer join tbl_post p on m.id = p.member_id;
